#!/usr/bin/env python3
"""
Install thin Orchestra AI skill wrappers into user-level Claude and Codex
skill directories.
...
def _relative_wrapper_path(agent: str, wrapper_name: str) -> str:
    return f".{agent}/skills/{wrapper_name}/SKILL.md"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and Codex skill directories. Normally run this as "
            "`ko-install-global-skills`."
        )
    )
    parser.add_argument(
        "--orchestra-dir",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Home directory whose .claude/skills and .codex/skills trees "
            "receive wrappers. Defaults to the current user home."
        ),
    )
    parser.add_argument(
        "--check",
