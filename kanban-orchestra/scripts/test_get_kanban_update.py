...
class TestBuildUpdate(unittest.TestCase):
    ...
    def test_running_dashboard_url_is_shown(self):
        ...
        with patch.object(get_kanban_update.fleet, "tailscale_dashboard_url", return_value=None):
            update = get_kanban_update.build_update(self.conn)

        self.assertIn("dashboard: http://127.0.0.1:8432", update)
        self.assertNotIn("Remote:", update)

    def test_running_dashboard_remote_url_is_shown_when_serve_mapping_exists(self):
        identity = db.get_instance_identity(self.db_path)
        dashboard_path = Path(identity["runtime_root"]) / "dashboard.json"
        dashboard_path.parent.mkdir(parents=True, exist_ok=True)
        dashboard_path.write_text(
        ...
        self.assertIn(
            "dashboard: http://127.0.0.1:8432\n  Remote: https://node.example.ts.net:8432/",
            update,
        )
